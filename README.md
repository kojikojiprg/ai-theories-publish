...
分割時、画像フォルダ(`images/<slug>/`)は分割せず両記事で共有参照します。また、公開前後のURLがまだ確定していないため、本文中に相互リンク用のTODOコメント(`<!-- TODO: ... -->`)が残ります。**公開後に、このTODOコメント部分を実際のURLへ手動で置き換えてください**。分割されない場合は、従来通り単一記事(`<slug>.md`)として生成されます。
...
- 前編・後編に分割された記事の場合、公開後に本文中のTODOコメント箇所を相互リンク(実際のURL)に置き換える
