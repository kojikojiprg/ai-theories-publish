...
**この注記は、本番実行の後に判明した事実の記録である。前提条件 P0 の宣言(期待値 1.6707、相対誤差 1% 以内)と
判定(成立)は変更しない。**

- 6.1 節の P0 の「許容誤差の根拠」にある「Hub のリポジトリは 008 のノートブックの実行後に作り直されており、008 の
  セル出力を生んだ実行と重みが bit 単位で同一であることは確認できない」という記述は、事実と異なっていた。
  リポジトリの作り直しは、008 の本番実行自身のアップロードセル(008 の 5.12 節)によるものである。008 の本番実行の
  セル出力には、リポジトリを削除して作り直した旨の出力が残っている。Hub のモデルリポジトリのコミット
  (2026-09-13 09:29 JST)は、その出力を含む 008 のノートブックのコミット(`4cb5890`、同日 10:13 JST)と整合する。
  Hub の`main`の重みは、008 が保存した 2,181 ステップ時点の重みである。
- 相対差 0.1576% は、期待値として転記した 008 の値 1.6707 が、学習途中の 2,000 ステップ時点の評価値であることに
  よる。008 の学習ループは 200 ステップごとにのみ検証を行い、学習ステップ数 2,181 は 200 で割り切れないため、
  印字された「最終検証 bits-per-byte」は 2,000 ステップ時点の値になっていた(008 の 5.10 節の注記を参照)。
  差は 2,000〜2,181 ステップの学習の進み分である。
- 008 と同じ設定でノートブックの外で学習し直し、2,000 ステップ時点で 1.670601、2,181 ステップ時点で 1.667984 を
  得た。それぞれ 1.6707・1.668067 と実行環境による差の範囲(約 1e-4)で一致する。また、008 の時点のコミットの
  `src/`で Hub の重みを評価しても 1.668067 であり、評価コード・検証集合・トークナイザは 008 の時点から
  変わっていない。
- P0 は「起点モデルの読み込みが正しいこと」を確かめる条件であり、上記の差の原因は読み込みの誤りではない。
  P0 の判定(成立)は変わらない。
...
![png](https://raw.githubusercontent.com/kojikojiprg/ai-theories-publish/main/images/013_quantization_basics/output_33_1.png)
