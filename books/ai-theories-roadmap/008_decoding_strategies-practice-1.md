...
### 5.10 節の注記(後日の調査による): 印字した検証 bits-per-byte の時点

**この注記は、本番実行の後に判明した事実の記録である。上の 2 つのコードセルとその出力は変更していない。**

- 上のセルが「最終検証 bits-per-byte = 1.6707」として印字した値は、学習を最後まで終えた時点の値ではない。
  `train_language_model()`は`step % EVAL_INTERVAL == 0`を満たすステップでのみ検証を行う。`NUM_STEPS = 2181`は
  `EVAL_INTERVAL = 200`で割り切れないため、評価の最後は 2,000 ステップ時点になる。印字した
  `train_history["eval_bits_per_byte"][-1]`は、この 2,000 ステップ時点の評価値である。
- 保存・アップロードした重み(`MODEL_STATE_PATH`と`kojikojiprg/ai-theories-small-gpt-en`の`model_state.pt`)は、
  2,181 ステップの学習を終えた時点のものである。同じ検証集合(同じ分割・非重複窓)でのこの重みの bits-per-byte は
  **1.6681**(1.668067)であり、010 と 013 のセル出力で確認されている。
- 同じ設定(学習率 1.2e-3、gradient clipping 閾値 0.6341、seed 42、2,181 ステップ)でノートブックの外で学習し直し
  (Apple の GPU バックエンド Metal Performance Shaders 上)、両時点の値を確認した。2,000 ステップ時点は 1.670601、
  2,181 ステップ時点は 1.667984 であり、それぞれ 1.6707・1.668067 と実行環境による差の範囲(約 1e-4)で一致した。
  この再現はノートブックの外で行ったものであり、このノートブックのセル出力には含まれない。
- 前提条件 P0 の判定は 2,000 ステップ時点の値 1.6707 で行っている。2,181 ステップ時点の値 1.6681 も閾値 2.5 を
  下回るため、成否は変わらない。
- 7 節の生成実験は、学習を終えた`model`(2,181 ステップ時点の重み)をそのまま使っている。7 節の結果はこの注記の
  影響を受けない。
