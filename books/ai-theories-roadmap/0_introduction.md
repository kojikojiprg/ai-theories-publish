...
| --- | --- | --- | --- | --- |
| 001 | [注意機構(Attention Mechanism)](https://zenn.dev/kojikojiprg/books/ai-theories-roadmap/viewer/001_attention_mechanism) | 01_foundations | なし / None | Query / Key / Value、Scaled Dot-Product Attention とスケーリング係数 $\sqrt{d_k}$ の導出、因果マスク、多頭注意機構(Multi-Head Attention)。実装は`src/layers/attention.py`にスクラッチ実装し、重みの可視化と copy task の学習で検証する |
...
| 004 | 正規化と活性化の系譜(🚧 準備中) | 01_foundations | 002 | 002 でスクラッチ実装した層正規化(Layer Normalization)を起点に RMSNorm への変遷、GELU から SwiGLU への活性化関数の変遷を辿り、現代 LLM がこれらの構成を採用する理由を考察する |
...
| 007 | 学習の安定化(🚧 準備中) | 02_pretraining | 006 | 002 で観測した正規化後置の勾配の不均衡を踏まえ、AdamW、warmup + cosine スケジュール、gradient clipping、mixed precision など、大規模言語モデルの学習を安定化させる技術を扱う |
