...
正規化・活性化関数のさらなる発展(RMSNorm、SwiGLU など)は [004](./004_normalization_activation.ipynb) (未作成)で、位置エンコーディングの各方式の比較・回転位置エンコーディング(RoPE)は [003](https://zenn.dev/kojikojiprg/articles/003_positional_encoding_rope-theory) (未作成)で、学習を安定化させる warmup などの最適化技術は 007(未作成)で扱う。
